...
def multi_head_attention_forward(query,                           # type: Tensor
    ...
    tgt_len, bsz, embed_dim = query.size() # length??batch_size??embedding dims
    assert embed_dim == embed_dim_to_check
    assert key.size() == value.size()

    head_dim = embed_dim // num_heads
    assert head_dim * num_heads == embed_dim, "embed_dim must be divisible by num_heads"
    scaling = float(head_dim) ** -0.5 # scale factor

    # ???????QKV???
    if not use_separate_proj_weight:
        if torch.equal(query, key) and torch.equal(key, value):
            # self-attention ???q/k/v
            # ???????��?????????????3???? ????input=query weight=in_proj_weight bias=in_proj_bias
    ...
    # torch.view()??????resize()
    # q/k/v size = [batch_size*num_heads, tgt_len, head_dim] 
