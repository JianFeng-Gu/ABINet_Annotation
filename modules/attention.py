...
class Attention(nn.Module):
    ...
    def forward(self, enc_output):
        ...
        # generate vector[0:max_length-1]
        reading_order = torch.arange(self.max_length, dtype=torch.long, device=enc_output.device)
        # expand an new dimension and broadingcast the size into [enc_output.size(0), maxlength]
...
# encoder layer: conv2d + BN + ReLu
def encoder_layer(in_c, out_c, k=3, s=2, p=1):
    return nn.Sequential(nn.Conv2d(in_c, out_c, k, s, p),
                         nn.BatchNorm2d(out_c),
                         nn.ReLU(True))

# decoder layer: Upsample + conv2d + BN + ReLu
# 
...
class PositionAttention(nn.Module):
    ...
    # x: features from backbone (output from transformer module)
    def forward(self, x):
        N, E, H, W = x.size()
        k, v = x, x  # (N, E, H, W)
        ...
        for i in range(0, len(self.k_encoder)):
            ...
            # save features from encoder layers
            features.append(k)
        for i in range(0, len(self.k_decoder) - 1):
            # range(0, 3) i=0,1,2 
            # i=0: k=self.k_decoder[0](k)+features[2] 
            # i=1: k=self.k_decoder[1](self.k_decoder[0](k)+features[2])+features[1]
            # i=2: k=self.k_decoder[2](self.k_decoder[1](self.k_decoder[0](k)+features[2])+features[1])+features[0]

            k = self.k_decoder[i](k)
            k = k + features[len(self.k_decoder) - 2 - i]
        k = self.k_decoder[-1](k) # k.size = [batch_size, channel, height, weigth]
        ...
        q = self.project(q)  # (N, T, E) E:channel N:batch_size T:max_length
        
        # calculate attention
        # flatten the images[batch_size, channel, H*W]
        attn_scores = torch.bmm(q, k.flatten(2, 3))  # (N, T, (H*W))    
        # H*W is equal to NLP embedding
        # T is the maxlength
        attn_scores = attn_scores / (E ** 0.5)
        # Get the weight of every column (H*W) by softmax
